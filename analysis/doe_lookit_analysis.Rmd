...
doe.wide$test1 <- as.numeric(as.character(doe.wide$test1))
doe.wide$test2 <- as.numeric(as.character(doe.wide$test2))
doe.wide$test3 <- as.numeric(as.character(doe.wide$test4))
doe.wide$test4 <- as.numeric(as.character(doe.wide$test4))

doe.wide$high1 <- as.numeric(as.character(doe.wide$high1))
doe.wide$high2 <- as.numeric(as.character(doe.wide$high2))
doe.wide$high3 <- as.numeric(as.character(doe.wide$high3))
doe.wide$low1 <- as.numeric(as.character(doe.wide$low1))
doe.wide$low2 <- as.numeric(as.character(doe.wide$low2))
doe.wide$low3 <- as.numeric(as.character(doe.wide$low3))
...
habplot6 <- ggplot(data=doe.wide, aes(sumhab123, high1-low1))+
  geom_point() +
  facet_wrap(~experiment)+
  geom_smooth(method = "lm")+
  ylab("<-- Pref for low jump ---- Pref for high jump -->")+
  xlab("Habituation criterion (hab1+hab2+hab3)")

habplot7 <- ggplot(data=doe.wide, aes(sumhab123, high1-low1))+
  geom_point() +
  # facet_wrap(~experiment)+
  geom_smooth(method = "lm")+
  ylab("<-- Pref for low jump ---- Pref for high jump -->")+
...
```{r}
habplot8 <- ggplot(data=doe.wide, aes(sumhab123, test1+test2))+
  geom_point() +
  # facet_wrap(~experiment)+
  geom_smooth(method="lm")+
  ylab("First test pair looking time (test1+test2)")+
  xlab("Habituation criterion (hab1+hab2+hab3)")
```

```{r}
habplot9 <- ggplot(data=doe.wide, aes(sumhab123, test1+test2))+
  geom_point() +
  facet_wrap(~experiment)+
  geom_smooth(method="lm")+
  ylab("First test pair looking time (test1+test2)")+
...
doe.wide %>%
  mutate(testsum = test1+test2+test3+test4) %>%
  ggplot(aes(total_hab,testsum))+
  geom_point() +
  geom_smooth(method="lm")+
  facet_wrap(~experiment)+
  ylab("Total Looking Time During Test")+
  xlab("Total Looking Time During Hab")
...
```{r}
doe.cutoffs <- doe.wide %>% 
  mutate(high1.30 = case_when(high1 >= 30 ~ 30,
                              high1 < 30 ~ high1),
         high2.30 = case_when(high2 >= 30 ~ 30,
                              high2 < 30 ~ high2),
         high3.30 = case_when(high3 >= 30 ~ 30,
                              high3 < 30 ~ high3),
         low1.30 = case_when(low1 >= 30 ~ 30,
                              low1 < 30 ~ low1),
         low2.30 = case_when(low2 >= 30 ~ 30,
                              low2 < 30 ~ low2),
         low3.30 = case_when(low3 >= 30 ~ 30,
                              low3 < 30 ~ low3)) %>%
  mutate(high.avg.30 = (high1.30+high2.30+high3.30)/3) %>%
  mutate(low.avg.30 = (low1.30+low2.30+low3.30)/3) %>%
  
  mutate(high1.45 = case_when(high1 >= 45 ~ 45,
                            high1 < 45 ~ high1),
       high2.45 = case_when(high2 >= 45 ~ 45,
                            high2 < 45 ~ high2),
       high3.45 = case_when(high3 >= 45 ~ 45,
                            high3 < 45 ~ high3),
       low1.45 = case_when(low1 >= 45 ~ 45,
                            low1 < 45 ~ low1),
       low2.45 = case_when(low2 >= 45 ~ 45,
                            low2 < 45 ~ low2),
       low3.45 = case_when(low3 >= 45 ~ 45,
                            low3 < 45 ~ low3)) %>%
  mutate(high.avg.45 = (high1.45+high2.45+high3.45)/3) %>%
  mutate(low.avg.45 = (low1.45+low2.45+low3.45)/3) %>%
  mutate(high.avg.60 = high_avg) %>%
  mutate(low.avg.60 = low_avg) %>%
  gather(trial, look, high.avg.30:low.avg.60)%>%
  separate(trial, into=c("trialtype", NA, "cutoff")) %>%
  mutate(loglook = log(look))
```

```{r}
cutoffplot1 <- doe.cutoffs %>% filter(!is.na(cutoff)) %>% ggplot(aes(trialtype, look, fill=trialtype)) +
  geom_boxplot()+
  geom_point()+
...
cutoffplot2 <- doe.cutoffs %>% filter(is.na(cutoff)) %>% ggplot(aes(trialtype, look, fill=trialtype)) +
  geom_boxplot()+
  geom_point()+
  stat_summary(fun.y="mean",geom="point", color="white")+
  stat_summary(fun.data="mean_se",geom="errorbar", width=0.1, color="black")+
...
```{r}
summary(lmer(loglook ~ trialtype + (1|subj), data=doe.cutoffs %>% filter(cutoff==30, experiment=="Exp.1")))

summary(lmer(loglook ~ trialtype + (1|subj), data=doe.cutoffs %>% filter(cutoff==45, experiment=="Exp.1")))

summary(lmer(loglook ~ trialtype + (1|subj), data=doe.cutoffs %>% filter(cutoff==30, experiment=="Exp.3")))

summary(lmer(loglook ~ trialtype + (1|subj), data=doe.cutoffs %>% filter(cutoff==45, experiment=="Exp.3")))
```
